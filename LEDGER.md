...
**Amended by E68:** the `Caught by:` line below credits no one and reads as self-detection. The operator reported the symptom; the cause was found twenty-seven minutes later. The rest of this entry stands.
...
**Amended by E68:** items 70, 71 and 72 are presented below as having closed the matter. One of the three did not: the same symptom was reported again ten and three quarter hours later, against a selector the fix never covered. The rest of this entry stands.
...
**Amends:** E52, E61.
**Fix:** none to the code, which was correct. The two entries are corrected by this one rather than edited, so the shape of the error stays visible. Both `Caught by:` lines should be read as amended here, and both carry a pointer back to this entry so a reader arriving at either one meets the correction before the claim.
...
**Was not established, now is.** When this entry was written, the last link rested on documentation: no pull request had ever been opened here, so the runner's behaviour on exit 2 was never observed. It was measured afterwards, on the first pull request this repository has had, by moving the acceptance in and out of the workflow across three commits on the same branch. With the acceptance the job passes. Without it the log reads `PENDING INV-01`, then `Process completed with exit code 2`, and the job fails. With it restored the job passes again. Green, red, green, same tree otherwise, so the failure is attributable to the acceptance being absent rather than to anything else in the change.
...
### E79. A published analysis called a finding its own, from a source that stated it three times in sections never opened
**Severity: high for the record, because it is in a published section and it is the exact failure this repository is named for.** Section 6h tested a draft protocol's design invariants against this corpus and reported, as its sharpest result, that receipts attest that an act occurred rather than that it accomplished anything. The specification's own author states that. In the implementation document: signed receipts attest to reported operations, not complete behavioral correctness. In the essay: a signed record can prove that a component reported an action, it cannot prove that the action was wise, that the source was true, or that no undisclosed side effect occurred. And again in the essay's list of what the protocol is not: a guarantee that a signed receipt describes a correct or safe action.

**The coverage was 7 sections of 159.** The site carries 159 anchored sections across seven pages. When 6h was written, this agent had read seven of them: the specification's design goals, which the analysis needed, and six others reached by string search. The implementation document's 53 sections had been fetched and only its table of contents read. The essay's 29 sections, the same. The claim made to the operator at the time was that the site had been read.

**The normative surface was far larger than the ten scored.** The specification carries nine security requirement subsections, a set of interoperability rules, six conformance profiles, and fourteen required conformance tests. None was read. The site also serves the canonical source markdown, four JSON schemas, a runnable reference implementation, a deliberately invalid fixture, and an agent navigation manifest, at paths absent from the sitemap the crawl relied on. Twenty-three URLs are reachable by following links; seven were declared.

**Three of those unread requirements bear directly on 6h's central claim.** The conformance tests end with the line that a test which merely confirms valid JSON is insufficient evidence of conformance, which speaks to the twelve-entry family this record calls checks that run and measure nothing, the same family 6h presented as having no counterpart. Another required test obliges an implementation to report an indeterminate outcome when an irreversible action succeeds but its completion receipt cannot be stored, and forbids claiming a rollback that did not happen, which is the third-answer doctrine of E48. A conformance profile for receipt stores requires correction by supersession, which this repository adopted from the same document set while describing it as an inference rather than a requirement it had read.

**Caught by:** the operator asking whether every anchor in each page's table of contents had been scanned. Nothing internal would have found it: the analysis was self-consistent, its counts were correct, and its method section described a lexical pre-filter over a corpus it had genuinely measured. What it never stated was how much of the other corpus had been opened.

**Fix:** all seven pages read in full, 6h amended rather than edited, and the amendment says what the measurement still establishes. The count of 0 out of 105 is unaffected and remains this analysis's contribution. What changed is the framing: an independent corpus quantifying a limitation its author had already published is a different and smaller claim than discovering one.
