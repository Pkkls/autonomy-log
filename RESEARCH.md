...
**Amended by E79, and read the amendment before the section.** Three claims below do not survive. The result called sharpest is stated by the author in three places this analysis never opened. The list of classes with no counterpart is wrong, because the normative surface is not the ten invariants scored here: nine security subsections, interoperability rules, six conformance profiles and fourteen required conformance tests were never read, and three of those requirements address the classes this section says are unaddressed. What survives is the counting method and the numbers themselves, which were measured correctly against the wrong denominator of the other document.
...
**Corrected after reading the whole source, per E79.** The section above scored ten invariants from one section and drew conclusions about a protocol whose normative surface is roughly four times that. Reading the rest overturns its most quotable paragraph.

The list of classes with no corresponding requirement is wrong on at least three counts. The conformance tests close on the statement that a test merely confirming valid JSON is insufficient evidence of conformance, which is precisely the twelve-entry family of checks here that execute correctly and observe nothing. A required test obliges an implementation to report an indeterminate outcome when an irreversible action succeeds and its completion receipt cannot be stored, and forbids claiming a rollback that did not occur, which is E48's three answers as a MUST. And a conformance profile for receipt stores requires correction by supersession, which is the rule this repository adopted from that document while presenting it as an inference.

What remains true is narrower and still worth having: against the ten design invariants, and only those, seven of 105 entries are prevented and none refuted. That is a measurement of one section, not of a protocol, and the section header above should be read as such.

**Two further things above are narrower than they read.**

The receipt result is not a discovery. The author states it in the implementation document's known limitations, in the essay's account of receipts, and in the essay's list of what the protocol is not. What this analysis contributes is the size: zero of a corpus in which the uncovered family is the largest one present, with the entries named. An independent record quantifying a published limitation is worth writing down and is not the same as finding it.

And ten invariants were not the whole normative surface. The same document set carries a verification checklist and a set of adapter requirements, both normative, neither examined here. One checklist item separates a receipt preflight failure from a post-action persistence failure entering an indeterminate recovery state, which is the same three-answer discipline this record argues for in E48, so the protocol holds a position this section implied it lacked. The adapter requirements include writing the required receipt before reporting final success, which is stronger than the invariant scored above. A fuller test would score those too; this section does not, and says so rather than presenting its ten as the whole.

### 6h.1 The same measurement, taken again fifteen days later

The section above was written against v0.1-draft on 2026-08-16 and stated, in its own words, that a specification still moving is exactly the kind of source a claim can outlive. The source published v0.2 on 2026-08-17, hours after the last commit here, and this record cited a version that no longer existed for the next fifteen days. Nothing caught it: the external link check verifies that a URL serves real content and refuses to be fooled by an application shell, which is a good answer to the question it asks, and the question it asks is not whether the document underneath the citation is still the one that was read. E81.

Re-measured at v0.2 rather than carried over. The ten design invariants are still ten and their names are unchanged, so the scoring above stands on the set it scored: 7 of 105 entries prevented, none refuted. Three things are new. `purpose_code` becomes the normative policy input, with optional purpose text that must not broaden authorization, which is the only one of the three this record has a use for and it is adopted in the LOOP section of AGENTS.md. The CL-Core-Lite profile requires a finite `expires_at` together with `single_use: true`, and expiry is unified across objects. Both are out of scope here for the same reason the vault and the bundle were: this record issues nothing to a consumer.
